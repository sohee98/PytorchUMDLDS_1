...
class BaseTrainer:
    ...
    def _preliminary_setups(self):
        ...
        if self.opt.no_cuda or not torch.cuda.is_available() or len(self.opt.gpus) == 0:
            ...
            torch.cuda.set_device(self.device)      # device(type='cuda', index=0)
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(trange):    
            ...
            # pdb.set_trace()
            ''' ## D1
                a1 = outputs[('depth', 0, 0)].detach().cpu().numpy()[0,0]
                a2 = inputs[('color', 0, 0)].detach().cpu().numpy()[0,0]
                a3 = outputs[('res_trans', 0, 1)].detach().cpu().numpy()[0,0]
                plt.figure(1); plt.imshow(a1, cmap='turbo'); plt.colorbar(); plt.ion(); plt.show()
                plt.figure(2); plt.imshow(a2); plt.colorbar(); plt.ion(); plt.show()
                plt.figure(3); plt.imshow(a3); plt.colorbar(); plt.ion(); plt.show()

            '''

            # prevent from having Nan values in the training
            if math.isnan(losses['loss'].item()):
                continue
            
            # pdb.set_trace()

            self.model_optimizer.zero_grad()        # grad 값 초기화
            ...
            if self.opt.log_frequency != 0:
                ...
                if self.opt.rank == 0:
                    ...
                    self.log_criteria("train", outputs)
                    # update the results by log_frequency, added by fei
        ...
        if self.opt.rank == 0:
            ...
            self.log_losses("train", mean_train_loss)   # tensorboard scalar 추가
            self.log("train", inputs, outputs)          # tensorboard image 추가
    ...
    def log_criteria(self, mode, outputs):
            """Log all the losses to tensorboard
            ...
            # pdb.set_trace()
            writer.add_scalars("depth_criteria", 
                {'max': outputs[('depth', 0, 0)].amax(dim=(1,2,3))[batch_id],
                 'min': outputs[('depth', 0, 0)].amin(dim=(1,2,3))[batch_id],
                 'mean': outputs[('depth', 0, 0)].mean(dim=(1,2,3))[batch_id],
                },
                self.step
            )
            # pdb.set_trace()
            ...
            tgt_frame_id = 0
            xyz = 0
            writer.add_scalars("translations_x",
                {'t2s_{}->{}_ego'.format(tgt_frame_id, src_frame_id):\
                    outputs[('translation', src_frame_id, tgt_frame_id)].mean(dim=(2,3))[batch_id, xyz],
                 't2s_{}->{}_res_max'.format(tgt_frame_id, src_frame_id):\
                    outputs[('res_trans', src_frame_id, tgt_frame_id)].amax(dim=(2,3))[batch_id, xyz],
                 't2s_{}->{}_res_min'.format(tgt_frame_id, src_frame_id):\
                    outputs[('res_trans', src_frame_id, tgt_frame_id)].amin(dim=(2,3))[batch_id, xyz],
                 's2t_{}->{}_ego'.format(src_frame_id, tgt_frame_id):\
                    outputs[('translation', tgt_frame_id, src_frame_id)].mean(dim=(2,3))[batch_id, xyz],
                 's2t_{}->{}_res_max'.format(src_frame_id, tgt_frame_id):\
                    outputs[('res_trans', tgt_frame_id, src_frame_id)].amax(dim=(2,3))[batch_id, xyz],
                 's2t_{}->{}_res_min'.format(src_frame_id, tgt_frame_id):\
                    outputs[('res_trans', tgt_frame_id, src_frame_id)].amin(dim=(2,3))[batch_id, xyz],
                },
                self.step
            )
            xyz = 1
            writer.add_scalars("translations_y",
                {'t2s_{}->{}_ego'.format(tgt_frame_id, src_frame_id):\
                    outputs[('translation', src_frame_id, tgt_frame_id)].mean(dim=(2,3))[batch_id, xyz],
                 't2s_{}->{}_res_max'.format(tgt_frame_id, src_frame_id):\
                    outputs[('res_trans', src_frame_id, tgt_frame_id)].amax(dim=(2,3))[batch_id, xyz],
            ...
            xyz = 2
            writer.add_scalars("translations_z",
                {'t2s_{}->{}_ego'.format(tgt_frame_id, src_frame_id):\
                    outputs[('translation', src_frame_id, tgt_frame_id)].mean(dim=(2,3))[batch_id, xyz],
                 't2s_{}->{}_res_max'.format(tgt_frame_id, src_frame_id):\
                    outputs[('res_trans', src_frame_id, tgt_frame_id)].amax(dim=(2,3))[batch_id, xyz],
            ...
            # writer.add_scalar("translation_x",
            #     outputs[('translation', src_frame_id, tgt_frame_id)][batch_id, xyz,:,:].mean()
            # )
            # writer.add_scalar("translation")
    ...
    def load_model(self):
        ...
        for n in self.opt.models_to_load:
            ...
            if n != "adam":
                ...
                optimizer_dict = torch.load(path, map_location=self.device)
