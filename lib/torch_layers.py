...
def get_motion_smooth_loss2(res_motion_map, motion_map):
    """Compute the smoothness of rotations and translations"""
    norm = 3.0 * torch.mean(torch.square(motion_map),
                            dim=[1, 2, 3], keepdim=True)
    motion_map = torch.div(res_motion_map, torch.sqrt(norm + 1e-12))
    return _motion_smooth_helper(motion_map)

def get_motion_sparsity_loss(res_motion_map, motion_map):
    """ Created by fei, to calculate motion sparsity loss
    """
    norm = 3.0 * torch.mean(torch.square(motion_map),
                            dim=[1, 2, 3], keepdim=True)
    motion_map = torch.div(res_motion_map, torch.sqrt(norm + 1e-12))
    return _motion_sparsity_helper(motion_map)
...
def _motion_sparsity_helper(motion_map):
    """ Created by fei, refered from: 
    Args:
      motion_map: A tensor to be sparsified, of shape [B, C, H, W]
    """
    tensor_abs = torch.abs(motion_map)
    mean = torch.mean(tensor_abs, dim=[2,3], keepdim=True).detach()
    # tf.reduce_mean(2 * mean * tf.sqrt(tensor_abs / (mean + 1e-24) + 1))
    return torch.mean(2 * mean * torch.sqrt(tensor_abs / (mean + 1e-24) + 1))

def _motion_sparsity_helper2(motion_map, weights=[10,5,1]):
    """ Created by fei, refered from: 
    Args:
      motion_map: A tensor to be sparsified, of shape [B, C, H, W]
    """
    ...
    tensor_abs = torch.abs(motion_map)
    mean = torch.mean(tensor_abs, dim=[2,3], keepdim=True).detach()
    sparse_map = 2 * mean * torch.sqrt(tensor_abs / (mean + 1e-24) + 1)
    sparse_cost = sparse_map[:,0] * weights[0] +\
                  sparse_map[:,1] * weights[1] +\
                  sparse_map[:,2] * weights[2]
    return torch.mean(sparse_cost)
